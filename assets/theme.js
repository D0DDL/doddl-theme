...
var FreeShippingBar = class extends HTMLElement {
  static get observedAttributes() {
    return ["threshold", "threshold-2", "total-price"];
  }
  constructor() {
    super();
    this._onCartChangedListener = this._onCartChanged.bind(this);
  ...
  async connectedCallback() {
    ...
    this.threshold2 = Math.round(this.threshold2 * (Shopify.currency.rate || 1));
    this._updateThresholdAmounts();
    document.addEventListener("cart:change", this._onCartChangedListener);
  }
  disconnectedCallback() {
    document.removeEventListener("cart:change", this._onCartChangedListener);
  ...
  get threshold() {
    return parseFloat(this.getAttribute("threshold")) || 0;
  }
  get threshold2() {
    return parseFloat(this.getAttribute("threshold-2")) || 0;
  }
  set threshold(value) {
    this.setAttribute("threshold", value);
  }
  set threshold2(value) {
    this.setAttribute("threshold-2", value);
  }
  get totalPrice() {
  ...
  async attributeChangedCallback(attribute, oldValue, newValue) {
    ...
    // Use the higher threshold as the max value for the progress bar
    const maxThreshold = Math.max(this.threshold, this.threshold2);
  ...
  _updateThresholdAmounts() {
    const threshold1Element = this.querySelector('[data-threshold="1"]');
    const threshold2Element = this.querySelector('[data-threshold="2"]');
    
    if (threshold1Element && this.threshold > 0) {
      threshold1Element.textContent = this._currencyFormatter.format(this.threshold / 100);
    }
    
    if (threshold2Element && this.threshold2 > 0) {
      threshold2Element.textContent = this._currencyFormatter.format(this.threshold2 / 100);
    }
  }
  _updateMessage() {
    const messageElement = this.querySelector("span");
    const currentTotal = this.totalPrice;
    const threshold1 = this.threshold;
    const threshold2 = this.threshold2;
    
    // Check if we have one or two thresholds
    const hasThreshold1 = threshold1 > 0;
    const hasThreshold2 = threshold2 > 0;
    
    if (hasThreshold1 && hasThreshold2) {
      // Both thresholds active - existing two-threshold logic
      const maxThreshold = Math.max(threshold1, threshold2);
      const minThreshold = Math.min(threshold1, threshold2);
      
      if (currentTotal >= maxThreshold) {
        messageElement.innerHTML = this.getAttribute("reached-message-2");
      } else if (currentTotal >= minThreshold) {
        const remaining = maxThreshold - currentTotal;
        const replacement = `<span class="bold text-accent">${this._currencyFormatter.format(remaining / 100).replace(/\$/g, "$$$$")}</span>`;
        messageElement.innerHTML = this.getAttribute("unreached-message-2").replace(new RegExp("({{.*}})", "g"), replacement);
      } else {
        const remaining = minThreshold - currentTotal;
        const replacement = `<span class="bold text-accent">${this._currencyFormatter.format(remaining / 100).replace(/\$/g, "$$$$")}</span>`;
        messageElement.innerHTML = this.getAttribute("unreached-message").replace(new RegExp("({{.*}})", "g"), replacement);
      }
    } else if (hasThreshold1) {
      // Only threshold 1 active
      if (currentTotal >= threshold1) {
        messageElement.innerHTML = this.getAttribute("reached-message");
      } else {
        const remaining = threshold1 - currentTotal;
        const replacement = `<span class="bold text-accent">${this._currencyFormatter.format(remaining / 100).replace(/\$/g, "$$$$")}</span>`;
        messageElement.innerHTML = this.getAttribute("unreached-message").replace(new RegExp("({{.*}})", "g"), replacement);
      }
    }
  ...
  attributeChangedCallback(attribute, oldValue, newValue) {
    window.customElements.whenDefined("progress-bar").then(() => {
      const progressBarElement = this.querySelector("progress-bar");
      
      // Determine which threshold to use as max
      const threshold1 = this.threshold;
      const threshold2 = this.threshold2;
      const hasThreshold1 = threshold1 > 0;
      const hasThreshold2 = threshold2 > 0;
      ...
      let maxThreshold;
      if (hasThreshold1 && hasThreshold2) {
        maxThreshold = Math.max(threshold1, threshold2);
      } else if (hasThreshold1) {
        maxThreshold = threshold1;
      } else {
        maxThreshold = 0; // Fallback if no thresholds
      }
